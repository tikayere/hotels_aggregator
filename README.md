...
| `app/services/payment.py` | `PaymentGateway` interface with two implementations — `StripePaymentGateway` (real, auth-then-capture) and `MockPaymentGateway` (always succeeds). `get_payment_gateway()` picks Stripe when `STRIPE_SECRET_KEY` is set, else falls back to the mock with a loud warning — no code outside this file changes either way. |
